...
#include <ira_merge_laser_tools/laserscan_virtualizer_combinedConfig.h>

typedef pcl::PointCloud<pcl::PointXYZ> myPointCloud;

using namespace std;
using namespace pcl;
using namespace laserscan_virtualizer_combined;
...
public:
	...
	void reconfigureCallback(laserscan_virtualizer_combinedConfig &config, uint32_t level);
...
void LaserscanVirtualizer::reconfigureCallback(laserscan_virtualizer_combinedConfig &config, uint32_t level)
{
...
LaserscanVirtualizer::LaserscanVirtualizer()
{
	...
	if (!nh.getParam("/laserscan_virtualizer_combined/base_frame", base_frame))
		base_frame = "/cart_frame";

	if (!nh.getParam("/laserscan_virtualizer_combined/cloud_topic", cloud_topic))
		cloud_topic = "/cloud_pcd";

	nh.getParam("/laserscan_virtualizer_combined/output_laser_topic", output_laser_topic);
	nh.getParam("/laserscan_virtualizer_combined/virtual_laser_scan", virtual_laser_scan);
...
{
	ros::init(argc, argv, "laserscan_virtualizer_combined");

	LaserscanVirtualizer _laser_merger;

	dynamic_reconfigure::Server<laserscan_virtualizer_combinedConfig> server;
	dynamic_reconfigure::Server<laserscan_virtualizer_combinedConfig>::CallbackType f;
